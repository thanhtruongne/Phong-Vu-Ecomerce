    ...
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content">
                <div>
                    <form action="{{ route('private-system.management.{view}.update',${module}->id)) }}" method="POST" class="form-horizontal" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div style="display: flex;justify-content:space-between;width:100%;margin-top:20px">
                            <div class="col-lg-10">
                                     ...
                                     <div class="ibox-content">
                                              ...
                                              <div class="" style="margin-top: 5px">
                                                      <div class="form-group">                               
                                                          ...
                                                          <div class="col-sm-10">
                                                              <input type="text" value="{{ old('name',${module}->name) }}" name="name" class="form-control onchange_title_seo_input">
                                                          </div>         
                                                      </div>
                                                      @if ($errors->has('name'))
                                 ...
                                 <div>
                                     @include('backend.component.album',['data' => ${module},'name'=>'album'])
                                 </div>
                                 <div>
                                     @include('backend.component.seo',['data' => ${module}])
                                 </div>                 
                             </div>
                             @php
                                  $arrayCategories = [];
                                  $arrayPush = [];
                                  foreach (${module}->{module}_cataloge_{module} as $key => $value) {
                                      $arrayCategories[] =  $value->pivot->{module}_cateloge_id;
                                  };
                                  if(!empty($arrayCategories)) {
                                      foreach ($arrayCategories as $item) {
                                          $arrayPush[] = \App\Models\{module_model}Cataloge::findOrFail($item)->categories->id;
                                      }
                                  }
                             @endphp
                             <div class="col-lg-3">
                              <div style="height: 100%">
                                  <div class="ibox-content">
                                      ...
                                      <div class="form-group" style="margin-top: 8px"><label class="control-label">Chọn danh mục cha (*)</label>
                                          <div class="">
                                              <select class="form-control select2" name="post_cateloges_id">
                                                      <option value="1">Root</option>
                                                  @foreach ($categories as $category)
                                                      <option {{ old('post_cateloges_id',${module}->{module}_cataloge->categories->id) == $category->id ? 'selected' : '' }}  value="{{ $category->post_cataloge->id }}">
                                          ...
                                          <div class="">
                                              ...
                                              <select class="form-control select2" name="categories_sublist[]" multiple="multiple">                                         
                                                  @foreach ($categories as $key => $category)
                                                      ...
                                                      value="{{$category->post_cataloge->id}}">
                                                          {{ str_repeat('|---',($category->depth > 0) ? $category->depth : 0) }}{{ $category->name }}
                                                      </option>
                                                  @endforeach
                                              </select>
                                      ...
                                      <div class="form-group" style="margin-top: 40px"><label class="control-label">Hình ảnh nhóm bài viết (*)</label>
                                          <div class="ckfinder_2" style="border: 1px solid #ccc;cursor: pointer;" data-type="image">
                                              <input type="text" name="image" hidden value="{{ old('image',${ModuleName}->image) }}" >
                                              <img class="image_post_cataloge" style="width:100%" src={{ old('image',${ModuleName}->image) ?? "https://res.cloudinary.com/dcbsaugq3/image/upload/v1710723724/ogyz2vbqsnizetsr3vbm.jpg" }} alt="">
                                          </div>
                                          @if ($errors->has('image'))
                                          ...
                                          <div class="">
                                              <select class="form-control select2" name="status">
                                                  @foreach ($filter['create']['status'] as $key =>  $status)
                                                      <option {{ old('status',${ModuleName}->status) == $key ? 'selected' : '' }} value="{{ $key }}">{{ $status }}</option>
                                          ...
                                          <div class="">
                                              <select class="form-control select2" name="follow">
                                                  @foreach ($filter['create']['follow'] as $key =>  $follow)        
                                                      <option {{ old('follow',${ModuleName}->follow) == $key ? 'selected' : '' }}  value="{{ $key }}">{{ $follow }}</option>
                                                      @endforeach   
                                              </select>
                                          </div>
