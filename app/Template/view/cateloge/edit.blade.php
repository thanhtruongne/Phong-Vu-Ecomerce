    ...
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content">
                <div>
                    <form action="{{ route('private-system.management.{view}.update',${ModuleName}->id) }}" method="POST" class="form-horizontal" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div style="display: flex;justify-content:space-between;width:100%;margin-top:20px">
                            <div class="col-lg-10">
                                     ...
                                     <div class="ibox-content">
                                              ...
                                              <div class="" style="margin-top: 5px">
                                                      <div class="form-group">                               
                                                          ...
                                                          <div class="col-sm-10">
                                                              <input type="text" value="{{ old('name',${ModuleName}->name) }}" name="name" class="form-control onchange_title_seo_input">
                                                          </div>         
                                                      </div>
                                                      @if ($errors->has('name'))
                                              ...
                                              <div class="form-group">                               
                                                  ...
                                                  <div class="col-sm-10">
                                                      <textarea data-target="content" id="content" name="content" class="form-control ckEdition">
                                                          {!! old('content',${ModuleName}->content) !!}
                                                      </textarea>
                                                  </div>         
                                              </div>
                                 ...
                                 <div>
                                  @include('backend.component.album',['title' => $filter['update']['album'],'data' =>  ${ModuleName},'name'=>'album'])
                                 </div>
                                 <div>
                                     @include('backend.component.seo',['data' => ${ModuleName},'title' => $filter['update']['SEO-title']])
                                 </div>                 
                             </div>
                             <div class="col-lg-3">
                                          ...
                                          <div class="">
                                              <select class="form-control select2" name="categories_id">
                                                  ...
                                                  @foreach ($categories as $category)
                                                      <option {{ ${ModuleName}->categories->id == $category->id ? 'selected' : '' }}  value="{{ $category->id }}">
                                                          {{ str_repeat('|---',($category->depth > 0) ? $category->depth : 0) }}{{ $category->{module_translate}->name }}
                                                      </option>
                                                  @endforeach
                                              </select>
                                          </div>
                                          ...
                                          <div class="">
                                              <select class="form-control select2" name="status">
                                                  @foreach ($filter['update']['status'] as $key =>  $status)
                                                      <option {{ ${ModuleName}->status === $key ? 'selected' : '' }} value="{{ $key }}">{{ $status }}</option>
                                                  @endforeach                                   
                                              </select>
                                          </div>
                                          ...
                                          <div class="">
                                              <select class="form-control select2" name="follow">
                                                  @foreach ($filter['update']['follow'] as $key =>  $follow)
                                                      ...
                                                      <option {{ ${ModuleName}->follow == $key ? 'selected' : '' }} value="{{ $key }}">{{ $follow }}</option>
                                                  @endforeach   
                                              </select>
                                          </div>
