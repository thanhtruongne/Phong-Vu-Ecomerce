...
{
    protected $menuService , $menuRepositories ,$languageRepositories , $menuCatelogeRepositories , $menuCatelogeService;
    ...
    {  
        $trashedCount = $this->menuRepositories->trashed()->count();
        ...
        return view('backend.Page.menu.menu.index',compact('trashedCount','config','filter','menuCateloge'));
    }
    ...
    {
        if($this->menuService->create($request , $this->languageRepositories->getCurrentLanguage()->id) == true) {
    ...
    public function edit(string $id)
    {
        ...
        [
            'menu' => function($query) {
                return $query->where('parent','=',0)->withDepth()->with('children')->get()->toFlatTree();
            }
        ]);
    ...
    public function children(int $id) {
        ...
        $menu = $this->menuRepositories->findByid($id,['*'],['menu_translate' => function($query) use($language_id) {
    ...
    {
        if($this->menuService->saveChildren($request , $this->languageRepositories->getCurrentLanguage()->id) == true) {
            return redirect()->route('private-system.management.menu.index')->with('success','Tạo menu thành công');
        }
    ...
    //    dd($json , $menu_cateloge_id);
       $flag = $this->menuService->SaveTheNestedTableListDynamic(
         $json ,
         $this->languageRepositories->getCurrentLanguage()->id,0,
         $menu_cateloge_id
       );
    }
