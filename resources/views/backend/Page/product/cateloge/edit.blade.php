    ...
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            <div class="ibox-title" style="min-height:60px">
                {{ Breadcrumbs::render('product.cateloge.edit') }}           
                <h5 style="margin-top: 6px">{{ $filter['update']['title'] }}</h5>
            </div>
            <div class="ibox-content">
                <div>
                    <form action="{{ route('private-system.management.product.cateloge.update',$productCateloge->id) }}" method="POST" class="form-horizontal" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div style="display: flex;justify-content:space-between;width:100%;margin-top:20px">
                            <div class="col-lg-10">
                                     ...
                                     <div class="ibox-content">
                                              ...
                                              <div class="" style="margin-top: 5px">
                                                      <div class="form-group">                               
                                                          <label class="col-sm-2 control-label">Tiêu đề nhóm (*)</label>
                                                          <div class="col-sm-10">
                                                              <input type="text" value="{{ old('name',$productCateloge->name) }}" name="name" class="form-control onchange_title_seo_input">
                                                          </div>         
                                                      </div>
                                                      @if ($errors->has('name'))
                                              ...
                                              <div class="form-group">                               
                                                  ...
                                                  <div class="col-sm-10">
                                                      <textarea data-target="content" id="content" name="content" class="form-control ckEdition">
                                                          {!! old('content',$productCateloge->content) !!}
                                                      </textarea>
                                                  </div>         
                                              </div>
                                 ...
                                 <div>
                                  @include('backend.component.album',['title' => $filter['update']['album'],'data' =>  $productCateloge,'name'=>'album'])
                                 </div>
                                 <div>
                                     @include('backend.component.seo',['data' => $productCateloge,'title' => $filter['update']['SEO-title']])
                                 </div>                 
                             </div>
                             <div class="col-lg-3">
                                  ...
                                  <div class="ibox-content">
                                          ...
                                          <div class="">
                                              <select class="form-control select2" name="categories_id">
                                                  ...
                                                  @foreach ($categories as $category)
                                                      <option {{ $productCateloge->id == $category->id ? 'selected' : '' }}  value="{{ $category->id }}">
                                      ...
                                      <div class="form-group" style="margin-top: 40px"><label class="control-label">Hình ảnh nhóm bài viết (*)</label>
                                          <div class="ckfinder_2" style="border: 1px solid #ccc;cursor: pointer;" data-type="image">
                                              <input type="text" name="image" hidden value="{{ old('image',$productCateloge->image) }}" >
                                              <img class="image_post_cataloge" style="width:100%" src={{ old('image',$productCateloge->image) ?? "https://res.cloudinary.com/dcbsaugq3/image/upload/v1710723724/ogyz2vbqsnizetsr3vbm.jpg" }} alt="">
                                          </div>
                                          @if ($errors->has('image'))
                                          ...
                                          <div class="">
                                              <select class="form-control select2" name="status">
                                                  @foreach ($filter['update']['status'] as $key =>  $status)
                                                      <option {{ $productCateloge->status === $key ? 'selected' : '' }} value="{{ $key }}">{{ $status }}</option>
                                                  @endforeach                                   
                                              </select>
                                          </div>
                                          ...
                                          <div class="">
                                              <select class="form-control select2" name="follow">
                                                  @foreach ($filter['update']['follow'] as $key =>  $follow)
                                                      ...
                                                      <option {{ $productCateloge->follow == $key ? 'selected' : '' }} value="{{ $key }}">{{ $follow }}</option>
                                                  @endforeach   
                                              </select>
                                          </div>
